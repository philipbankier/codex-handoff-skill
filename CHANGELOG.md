...
### Added
- Simple walkthrough example (`examples/simple/`) with complete annotated transcript of the codex-handoff loop
- Advanced momentum-trader example (`examples/advanced-momentum-trader/`) demonstrating multi-phase plan execution
- Examples section in README with links to both examples

### Changed
- Moved momentum-trader example from `example/` to `examples/advanced-momentum-trader/` (cleaner structure, no .venv/.git artifacts)
- Updated Repo Structure tree in README to reflect new `examples/` directory
- Added `.venv/`, `__pycache__/`, `node_modules/`, `*.pyc` to `.gitignore`

### Removed
- Old `example/` directory (replaced by `examples/`)
